...
export default function solution(input) {
    ...
    const safeMod = product(Object.values(monkeys).map(m => m.divisible));
    for (let i = 0; i < 10000; i++) {
        for (const monkey of Object.values(monkeys)) {
            for (let item of monkey.items) {
                monkey.inspected++;
                item = monkey.op(item) % safeMod;
...
assertEqual(solution(testInput), 2713310158);
console.log(solution(input)); // 30616425600
