...
function solution(input) {
    ...
    const positions = scanners.map(s => s.position);
    let maxDist = 0;
    for (const [i, pos1] of enumerate(positions)) {
        for (const pos2 of positions.slice(i + 1)){
            const dist = sum(sub(pos1, pos2).map(x => Math.abs(x)));
...
function moveScanner(scanner, delta) {
    ...
    Object.assign(scanner.position, add(scanner.position, delta));
}

function rotate(scanner, rotation) {
    scanner.forEach(p => Object.assign(p, matMult(rotation, p)));
    Object.assign(scanner.position, matMult(rotation, scanner.position));
}
