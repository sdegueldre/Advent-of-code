...
import { sum, enumerate, neighbors, pairSum, enumGrid } from "../../utils.js";
...
export function solve(input) {
    ...
    const seen = new Set();
    const nums = [];
    for (const { y: i, x: j, row: line, cell } of enumGrid(grid)) {
        if (cell !== "*") {
            continue;
        }
        const adjNumbers = [];
        ...
        for (let [y, x] of neighbors.map(pairSum([i, j]))) {
            if (!grid[y][x].match(/\d/) || seen.has([x, y].join()) || tempSeen.has([x, y].join())) {
                continue;
            }
            let left = x;
            let right = x;
